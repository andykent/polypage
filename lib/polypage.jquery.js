...
(function($) {
  ...
  $.polypage = {
    ...
    buildOptionsBar: function(elems){
      ...
      var self=this;
      $(elems).append('<div id="pp_options"><a href="#close" class="closer">»</a><p>'+this.options.label+'</p><ul></ul></div>');
      ...
      $('#pp_options').click(function(event) {
        ...
        if(target.length) {
          if(target.attr('id')) {
            ...
            var options = $(this);
            self.options.closed = options.hasClass('closed');

            target.html(self.options.closed ? '»' : '«');
            options.toggleClass('closed', !self.options.closed)
          }
          event.preventDefault();
          event.stopPropagation();
        }
