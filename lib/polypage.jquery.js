...
(function($) {
  ...
  $.polypage.Base.prototype = {
    ...
    bindEvents: function(){
      ...
      if(this.options.enableClickEvents)
        this.scope.bind('click.pp', function(e) { pp.clickHandler(e); });
      // this.scope.bind('submit.pp', this.submitHandler);
    },
    ...
    triggerEventFromHash: function(href){
      if(href.search(/^#pp_/)==-1) return false;
      var href = href.replace(/^#pp_/,'');
      if(href.search(/^toggle_/)>=0) {
        this.scope.trigger('pp_toggleState', href.replace(/^toggle_/, ''));
        return true
      }
      if(href.search(/^set_/)>=0) {
        var val = href.search(/_false$/)==-1 ? true : false
        href = href.replace(/^set_/, '')
        href = href.replace(/_true$|_false$/, '')
        var obj = {}
        obj[href]=val
  ...
  $.polypage.NavBar.prototype = {
    ...
    bindEvents: function() {
      var nb = this;
      this.scope.bind('pp_forceNavBarRedraw', function() { nb.redraw() });
    },
    ...
    redraw: function(){
      this.scope.trigger('pp_navBarRedrawBegin', this);
      this.undraw();
      if(!this.needsOptionBar()) return;
      this.draw();
      this.scope.trigger('pp_navBarRedrawComplete', this);
    },
